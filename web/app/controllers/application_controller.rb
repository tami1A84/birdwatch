class ApplicationController < ActionController::Base
  ...
  # (buddy lists, garbage content) falls back to the person icon.
  ...
  # Rounded-square avatar with the kind-0 picture; person fallback (also
  ...
  def avatar_tag(pubkey, klass: "avatar--lg")
    ...
    icon = '<span class="msr msr--outline avatar__fallback">person</span>'
  ...
  def note_content(note)
    content, attached =
      if note.is_a?(Hash)
        [note["content"].to_s, imeta_images(note["tags"])]
      else
        [note.to_s, []]
      end
    ...
    html = parts.join.gsub(/\r?\n/, "<br>")

    # NIP-92 imeta attachments that don't ride in the content text.
    extras = attached.reject { |u| shown.include?(u) }
    unless extras.empty?
      imgs = extras.map { |u| note_image_tag(ERB::Util.html_escape(u)) }.join
      html += %(<span class="note-media">#{imgs}</span>)
    end
    html.html_safe
  end

  # imeta tag carries "url <URL>" pairs, in one element or several:
  # ["imeta", "url https://… m image/png", "x <sha>"].
  def imeta_images(tags)
    Array(tags).select { |t| t.is_a?(Array) && t[0] == "imeta" }.filter_map do |t|
      t[1..].join(" ")[/url (https?:\/\/\S+)/, 1]
    end
  end

  def note_image_tag(url)
    %(<a href="#{url}" class="note-media" target="_blank" rel="noopener noreferrer">) +
